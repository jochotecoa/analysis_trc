...
predictive.values = 'TRC' # TRC / targetRNA_TPM
minimum_samples = 3
is.shift = T
shift = c(1,2,3)
...
samples = getSamples(timepoints = timepoints, triplicates = triplicates, control = T)
...
# Get a filtered list of proteins
protvalcomp = proteinvaluesfile[,c(1,grep(comp, colnames(proteinvaluesfile)))] # Get all columns with our compound
protvalcomp = protvalcomp[!(grepl(protvalcomp[,1], pattern='\\:')),] # Exclude proteins with several IDs
...
shiftlist = NULL
naprotcorlist = NULL
pb = txtProgressBar(min = 0, max = nrow(protvalcompsel), style = 3)
init.time = proc.time()
  ...
  if (length(transel)==0) { 
    ...
    for (t in transel) {
      ...
      for (s in samples) {
        ...
        if (file.exists(samplefilepath)) {
          ...
          TRCselrow = grep(x = rownames(TRCscorefile), pattern = t)
          if(length(TRCselrow) > 0){
            TRCsel = TRCscorefile[TRCselrow,predictive.values] 
            if (length(transel)>1) {
              protTRCvalsel[2, grep(pattern = s, x = colnames(protTRCvalsel))] = protTRCvalsel[2, grep(pattern = s, x = colnames(protTRCvalsel))] + TRCsel
              
            } else {
              protTRCvalsel[2, grep(pattern = s, x = colnames(protTRCvalsel))] = TRCsel
            }
          } else {
          }
        } else {
    ...
    if (numcolsavail >= minimum_samples) {
      ...
      if (is.shift) {
        ...
        if (max(tempcorlist[,1], na.rm = T)>=-1) {
          corlist[length(corlist)+1] = max(tempcorlist[,1], na.rm = T)
          protlist[length(protlist)+1] = protnaam
          translist[length(translist)+1] = t[1]
          shiftlist[length(shiftlist)+1] = tempcorlist[grep(pattern = max(tempcorlist[,1]), x = tempcorlist[,1]), 2][1]
          
        } else {
          naprotcorlist = c(naprotcorlist, max(tempcorlist[,1], na.rm = T))
        }
      } else {
...
if (nrow(protvalcompsel) != length(corlist) + length(naprotcorlist) + length(protwouttrans) + length(protnotminexpr)) {
  print(paste0('Error: there are ', nrow(protvalcompsel) - (length(corlist) + length(naprotcorlist) + length(protwouttrans) + length(protnotminexpr)), ' proteins that have not been analyzed'))
}

corlistnames = cbind(translist,protlist,corlist)
setwd('/share/analysis/hecatos/juantxo/score_protein_analysis/')
write.table(x=corlistnames,file=paste0(comp,'_correlation_results_',predictive.values,'_protein_1onseveral_until072_minimumexpressedsamples-', minimum_samples, '_shift-', is.shift, '_timeps-shifted-', timepoints[timepoints.shifted], '.tsv'), sep='\t', quote=F, row.names=F)
...
barplot(table(cut(corlist, breaks = seq(from = -1, to = 1, by = 0.1))), las = 2, main = paste0('Correlation between ', predictive.values, ' and protein (Min.samples = ', minimum_samples, ')'), sub = paste0('Shift: max(', paste0(shift, collapse=','), ')'))
dev.off()
