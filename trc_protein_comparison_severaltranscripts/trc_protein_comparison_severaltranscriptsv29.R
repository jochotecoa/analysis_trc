...
##### Input Variables #####
script.name <- basename(strsplit(commandArgs(trailingOnly = FALSE)[4],
                                 "=")[[1]][2])
...
predictive.values.list = c('targetRNA_TPM') # TRC / targetRNA_TPM
...
protrans = read.csv(file = 'uniprot-yourlist_M20190501.csv2', 
                    header = T, 
                    stringsAsFactors = F)
  ...
  for (predictive.values in predictive.values.list) {
    for (minimum_samples in minimum_samples.list) {
      ...
      if (length(protwouttrans) > 0) {
        print(paste0('Warning: transcript not found for these proteins: ', 
                     paste(protwouttrans, collapse = ',')))
      }
      ...
      setOrCreatewd(time)
      corlistnames.filenaam = paste0(comp,
                                     '_correlation_results_',
                                     predictive.values,
                                     '_protein_1onseveral_until072_', 
                                     'minimumexpressedsamples-', 
                                     minimum_samples, 
                                     '_shift-', 
                                     is.shift, 
                                     '_timeps-shifted-', 
                                     timepoints[timepoints.shifted], 
                                     '.tsv')
      saveMetadata(x = metadata)
      write.table(x = corlistnames,file = corlistnames.filenaam, 
                  sep = '\t', 
                  quote = F, 
                  row.names = F)
      write.table(x = allprotTRCvalsel,
                  file = paste0(comp, 
                              '_dataframe_containing_', 
                              predictive.values, 
                              '_protein_1onseveral_until072_', 
                              'minimumexpressedsamples', minimum_samples,
                              '_shift-', is.shift, 
                              '_timeps-shifted-', 
                              timepoints[timepoints.shifted], '.tsv'), 
      ...
      setOrCreatewd(comp)
      setOrCreatewd(predictive.values)
      setOrCreatewd(dir.name = time)
      png(paste0('frequency_distribution_',
                 comp,
                 '_correlation_results_',
                 predictive.values,
                 '_protein_1onseveral_until072_minimumexpressedsamples-', 
                 minimum_samples,
                 '_shift-', 
                 is.shift, 
                 '_timeps-shifted-', 
      ...
      barplot(table(cut(corlist, 
              ...
              main = paste0('Correlation between ', 
                            ...
                            minimum_samples, ')'), 
              sub = paste0('Shift: max(', 
                           paste0(shift, collapse = ','), 
                           ')'), 
              ylim = c(0,120))
      dev.off()
