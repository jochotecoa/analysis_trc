...
pngPlotLine <- function(filename, x, y, lm, save = T, ...) {
  if (save) {
...
addNamesCol <- function(stacked.vector, original.table, col.ID.Name = 'ID', 
                        col.Data.Name = 'Data') {
  stacked.df = as.data.frame(stacked.vector)
  stacked.df[, col.ID.Name] = rep(x = rownames(original.table),
                                  times = ncol(original.table))
  colnames(stacked.df)[1] = col.Data.Name
  return(stacked.df)
}
...
prot.all = addNamesCol(stacked.vector = prot.all, 
                       original.table = protein.table)

filtered.data = filterData(tpm = tpm.all, 
                           prot.df = prot.all, 
                           prot.col = colnames(prot.all)[1], 
                           nonexpressed.rm = T)
...
if (kIntercept.fixed) {
  ...
  lm.transcprot = lm(formula = y ~ x)
}

a = function(x, y) cor(x, y) ^ 2
...
# # 
# # a = dput(df.data)
# # lm.transcprot = lm(formula = I(a[, 2] - 0) ~ 0 + a[, 1])
# # lm.transcprot
# # 
