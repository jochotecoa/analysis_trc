...
pb1 = txtProgressBar(min = 0, max = nrow(protvalcompsel), style = 3)
time = timestamp(prefix = paste(comp, 'trc_protein_comparison_severaltranscriptsv24.R', sep = '_'))
dir.create(time)
...
for (minimum_samples in minimum_samples.list) {
  ...
  setwd('/share/analysis/hecatos/juantxo/score_protein_analysis/plots/plots_correlations_nproteins_stringencies/')
  setOrCreatewd(comp)
  setOrCreatewd(paste0('shift_', is.shift))
  setOrCreatewd(time)
