...
# mart.human = useMart(biomart = marts$biomart[1], dataset = datasets$dataset[grep('hsapiens', datasets$dataset)])
# 
...
for (s in shifts) {
  ...
  mart.mouse = useMart(biomart = marts[1,1], dataset = datasets.mouse[2,1])
  filts = searchFilters(mart = mart.mouse, pattern = 'Protein stable ID')
  filt = filts$name[1]
  ...
  halflives[,10:ncol(halflives)] = as.data.frame(sapply(halflives[,10:ncol(halflives)], function(x) gsub(pattern = ',', replacement = '.', x)))
  halflives[,10:ncol(halflives)] = sapply(halflives[,10:ncol(halflives)], function(x) as.numeric(x))
...
for (col in 1:ncol(halflives)) {
  if (is.double(x = halflives[,col])) {
    ...
    for (ts in 0:3) {
      ...
      timesh.diff = mean(timesh[,col], na.rm = T) / mean(halflives[,col], na.rm = T)
      mean.diffs = c(meand.diffs, timesh.diff)
    }
    plot(x = mean.diffs, y = ls()[grep(pattern = '.halflives', x = ls())][-1], main = colnames(halflives)[col])
  }
}
