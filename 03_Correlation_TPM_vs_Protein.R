#' Correlation of TPM and Protein Expression
#' 
#' This script analyzes the correlation between transcriptomics (targetRNA_TPM) 
#' and proteomics data.
#'
#' Inputs:
...
predictive.values = 'targetRNA_TPM' 
minimum_samples = 12
is.shift = FALSE
shift = 0
...
if (file.exists(proteomicsfilepath)) {
  ...
  for (protrow in 1:nrow(protvalcompsel)) {
    ...
    if (length(transel) == 0) { 
      ...
      protTRCvalsel[2,1] = paste(protTRCvalsel[1,1],'TPM values')
