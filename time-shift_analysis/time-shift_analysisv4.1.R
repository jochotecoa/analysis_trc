...
setwd('plots/')
time = timestamp(prefix = 'plots time-shift_analysisv4.1.R ')
dir.create(time)
...
means = c()
for (ts in ls()[grep(pattern = '.halflives', x = ls())][-1]) {
  tish = get(x = ts)
  means = c(means, median(x = tish$Protein.half.life.average..h. , na.rm = T))
}
means
